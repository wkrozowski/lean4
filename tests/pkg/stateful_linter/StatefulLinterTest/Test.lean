...
/- Exercises the linter group in synchronous mode (set package-wide in the lakefile): each command
makes `emitter` count, and the three readers each report the count they read out of `emitter`'s
pre-phase handoff. The count threads across commands, so the reports increase.
