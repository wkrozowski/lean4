...
/-!
  Eta-reduction asymmetry with loose bvars in Sym.Simp.DiscrTree.

  `getMatchLoop` calls `etaReduce` during lookup, but `mkPathAux` does not during
  insertion. When a pattern lambda has loose bvars (from outer quantifiers), it is
  not eta-reducible and gets key `.other`. The corresponding closed expression lambda
  IS eta-reducible and gets key `.const`. This causes lookup to fail.
-/

open Lean Meta Sym Simp in
/--
info: disc tree lookup match count: 0
-/
#guard_msgs in
