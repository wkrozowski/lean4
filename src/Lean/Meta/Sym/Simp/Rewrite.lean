      ...
      if let .mvar mvarId := arg then
        if (← mvarId.isAssigned) then
          ...
          if let some val ← d decl.type then
            ...
            trace[Meta.Tactic] "Could not discharge {arg}"
            return .rfl
      else if arg.hasMVar then
        let arg ← instantiateMVarsS arg
        args := args.set i arg
    let mut proof := mkValue thm.expr thm.pattern us args.toArray
...
public def Theorem.rewriteMatchCongrThm (thm : Theorem) (e : Expr) (d : Discharger := dischargeNone) (heq := false) : SimpM Result :=
  /-
  **Note**: We use `withNewMCtxDepth` to ensure auxiliary metavariables used during the `match?`
  do not pollute the metavariable context.
  Thus, we must ensure that all assigned variables have be instantiate.
  ...
  withNewMCtxDepth do
  if let some result ← thm.pattern.match? e then
    trace[Meta.Tactic] "Successfully matched: {thm.expr}"
    -- **Note**: Potential optimization: check whether pattern covers all variables.
    let mut args := result.args.toVector
    let us ← result.us.mapM instantiateLevelMVars
    trace[Meta.Tactic] "Args: {args.toArray}"
    for h : i in *...args.size do
      let arg := args[i]
      if let .mvar mvarId := arg then
        if (← mvarId.isAssigned) then
        ...
        let arg ← instantiateMVarsS arg
        args := args.set i arg
    if (← args.allM fun x => x.mvarId!.isAssigned) then
      trace[Meta.Tactic] "Unsolved constraints"
      return .rfl
    args ← args.mapM fun arg => return ← instantiateMVarsS arg
