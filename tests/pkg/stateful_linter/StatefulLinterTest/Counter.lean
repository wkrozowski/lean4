...
/-! Registers a group of stateful linters. `emitter` counts commands: its `pre` produces a `Payload`
handoff (declining on terminal commands such as end-of-input), its `post` folds that into a persistent
`Counter`. `readerA`/`readerB`/`readerC` have no `pre` pass (the default) and no state others read
(their handles are discarded); their `post` reads `emitter`'s pre-phase `Payload` through `emitter`'s
handle — exercising several post phases consuming one pre phase, handle-based cross-linter reads,
`τ ≠ σ`, declining, and the default `pre`. Registration happens at import time, satisfying
`registerStatefulLinter`'s initialization-only guard. -/
...
initialize emitter : StatefulLinter Counter Payload ←
  registerStatefulLinter `StatefulLinterTest.emitter (Counter.mk 0)
    (pre := fun stx self _ =>
...
initialize
  registerReader emitter `StatefulLinterTest.readerA "reader A"
  registerReader emitter `StatefulLinterTest.readerB "reader B"
  registerReader emitter `StatefulLinterTest.readerC "reader C"
