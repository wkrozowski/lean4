...
-- def t₁ : DHashMap String (fun _ => String) := emptyWithCapacity.insertMany [⟨"a","b"⟩, ⟨"b", "c"⟩,  ⟨"d", "c"⟩]
-- def t₂ : DHashMap String (fun _ => String) := emptyWithCapacity.insertMany [⟨"a", "b_new"⟩, ⟨"z", "c"⟩]

-- #eval! (t₁.inter t₂).toList
